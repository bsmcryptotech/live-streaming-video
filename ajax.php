				...
				if ($player_page == 'detail' && $playlist_uniq_id != '')
				{
					...
					else
					{
						...
						foreach ($playlist_items as $k => $item)
						{
							...
							{
								$total_items = pm_count($playlist_items);
				...
				else
				{
					if ($video = request_video(trim($_POST['vid']))) // vid = 'uniq_id' in this case
					{
						$tags_arr = get_video_tags($video['uniq_id'], 1);
						 (pm_count($tags_arr) > 0) ? implode(',', $tags_arr) : '';
						
						if(pm_count($tags_arr) > 0)
						{
					...
					else
					{
						...
						if ($video_type == 'uploaded')
						{
							...
							if ($mysql_insert_id)
							{
								...
								if (is_array($row) && pm_count($row) > 0)
								{
	...
	case 'profile':
		...
		switch ($action)
		{
			case 'load-notifications':
				...
				{
					$smarty->assign('total_notifications', pm_count($notification_list));
			...
			case 'getfollowers': // return HTML
				...
				if ($followers_count)
				{
					...
					if (pm_count($list) > 0)
					{
						...
						$smarty->assign('total_profiles', pm_count($list));
			...
			case 'activity-stream':
				...
				$smarty->assign('total_activities', pm_count($activity_stream));
