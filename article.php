...
if ( ! _MOD_ARTICLE && ! (is_admin() || is_editor() || (is_moderator() && mod_can('manage_articles'))))
{
	redirect_404();
}
...
else
{
	...
	else
	{
		...
		if ($page == 1)
		{
			...
			if ((is_array($sticky) && pm_count($sticky) == 0)  || empty($sticky))
			{
				$sticky = false;
			}
			else
...
$smarty->assign('latest_articles', $latest_articles = art_load_articles(0, 5));
$smarty->assign('popular_articles', $popular_articles = art_load_articles(0, 5, '', '', array(), 'views'));
