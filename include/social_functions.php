...
function get_following_list($follower_id = -1, $start = 0, $limit = 30)
{
	...
	$sql = "SELECT f.user_id, u.username, u.name, u.gender, u.country, u.reg_date, u.last_signin, u.power, u.about, u.avatar, u.followers_count, u.following_count, u.channel_slug, u.channel_verified, u.channel_cover
			FROM pm_users_follow f 
	...
	while ($row = mysql_fetch_assoc($result))
	{
		...
		if ($row['channel_cover'] != '')
		{
			$leaders[$row['user_id']]['channel_cover'] = array('filename' => $row['channel_cover'],
													 'max' => _COVERS_DIR . $row['channel_cover'],
													 '450' => _COVERS_DIR . str_replace('-max.', '-450.', $row['channel_cover']),
													 '225' => _COVERS_DIR . str_replace('-max.', '-225.', $row['channel_cover']) 
													);
...
function check_multiple_relationships($user_ids = array(), &$followers, &$following)
{
	...
	if (pm_count($user_ids) == 0 || empty($userdata['id']))
	{
		return false;
	}
...
function log_activity($args = array())
{
	...
	if ($object_id != 0 && pm_count($object_data) > 0)
	{
		switch ($object_type)
		{
			case ACT_OBJ_USER:
	...
	if ($target_id != 0 && pm_count($target_data) > 0)
	{
		switch ($target_type)
		{
			case ACT_OBJ_USER:
	...
	if (! is_null($args['metadata']) && pm_count($args['metadata']) > 0)
	{
...
function get_activity_data($activity_id)
{
	...
	if (is_array($activity_id) && pm_count($activity_id) == 1)
	{
		$single = true;
...
function activity_stream_rollup($activity_data, &$actor_bucket, &$object_bucket, &$target_bucket, &$activity_meta_bucket) 
{
	...
	$activity_count = pm_count($activity_data);
	...
	for($i = 0; $i < $activity_count; $i++)
	{
		...
		if ($a['activity_type'] == ACT_TYPE_FOLLOW)
		{
			...
			$temp_count = pm_count($activity_data);
		...
		else if (in_array($a['activity_type'], array(ACT_TYPE_LIKE, ACT_TYPE_WATCH, ACT_TYPE_DISLIKE, ACT_TYPE_FAVORITE, ACT_TYPE_COMMENT, ACT_TYPE_UNFOLLOW)))
		{
			...
			$activity_meta_bucket[$i]['objects_count'] = pm_count($object_bucket[$i]);
			$activity_meta_bucket[$i]['targets_count'] = pm_count($target_bucket[$i]);
...
function suggest_who_to_follow($start_from = 0, $limit = 10)
{
	...
	$sql = "SELECT id, username, name, gender, country, reg_date, last_signin, power, avatar, followers_count, following_count, channel_slug, channel_cover, channel_verified 
			FROM pm_users 
			WHERE power IN ('".U_ACTIVE."', '".U_ADMIN."', '".U_MODERATOR."', '".U_EDITOR."') ";
	$sql .= (pm_count($ignore_user_ids) > 0) ? ' AND id NOT IN ('. implode(',', $ignore_user_ids) .') ' : '';
	...
	foreach ($final_list as $id => $u)
	{
		...
		$final_list[$id]['videos_count'] = count_entries('pm_videos', 'submitted', secure_sql($u['username'])  ."' AND added <= '". time() );
		$final_list[$id]['videos_count_formatted'] = pm_number_format($final_list[$id]['videos_count']);
	}
