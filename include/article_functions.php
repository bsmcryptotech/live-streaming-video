...
function insert_new_article($postarr)
{
	...
	if ( ! empty($postarr['post_thumb_show']))
	{
		add_meta($last_id, IS_ARTICLE, '_post_thumb', $postarr['post_thumb_show']);	
		add_meta($last_id, IS_ARTICLE, '_post_thumb_show', $postarr['post_thumb_show']);
	}
...
function update_article($id, $postarr)
{
	...
	if ( ! empty($postarr['post_thumb_show']))
	{
		update_meta($id, IS_ARTICLE, '_post_thumb', $postarr['post_thumb_show']);	
		update_meta($id, IS_ARTICLE, '_post_thumb_show', $postarr['post_thumb_show']);
	}
...
function delete_article($id)
{
	...
	if ( ! $result)
	{
		return array('type' => 'error', 'msg' => 'Could not delete this article. An error occurred: '. mysql_error());
	}
...
function mass_delete_articles($ids)
{
	...
	if (pm_count($ids) > 0)
	{
		...
		if ( ! $result)
		{
			return array('type' => 'error', 'msg' => 'Could not delete these articles. An error occurred: '. mysql_error());
		}
...
function art_insert_tags($id, $arr_tags)
{
	...
	$temp = array();
	for($i = 0; $total = pm_count($arr_tags), $i < $total; $i++)
	{
		if($arr_tags[$i] != '')
			if($i <= ($total-1))
			{
...
{	
	...
	if (is_array($array_of_ids) && pm_count($array_of_ids) > 0)
	{
...
function art_make_link($type = 'article', $args = array())
{
	...
	switch ($type)
	{
		case 'article':
			...
			{
				$url .= 'article-read.php?a='. $args['id'];
			}
		break;
...
function get_sticky_articles()
{
	...
	if (mysql_num_rows($result) > 0)
	{
		...
		$sticky = art_load_articles(0, pm_count($ids), '', 0, $ids);
