...
// +------------------------------------------------------------------------+
include('password_functions.php'); // PHP <5.5 compatibilty for password_*() functions.
...
function check_user_login_info($username, $user_key){
	...
	if( strcmp($user_key, md5($row['password'])) ) // note: $user_key is md5([any hashing algo])
		return 0;
...
function confirm_login( $username, $pass, $hashed = false ){
	...
	// check if passwords match
	if (strcmp($password, $row['password']) == 0) // md5 hash matches
	{
		if ( ! $hashed)
		{
			// update current md5 hash to a stronger hash
			$new_pass_hash = pm_password_hash($pass);
			$sql = "UPDATE pm_users 
					SET password = '". secure_sql($new_pass_hash) ."' 
					WHERE id = ". $row['id'];
			mysql_query($sql);
		}
	...
	return (pm_password_verify($pass, $row['password'])) ? 1 : 0;
}
...
function reset_password($email_address = ''){
	...
	// $new_pass_hash = pm_password_hash($new_pass);
	$new_pass_hash = md5($new_pass); // md5() because pm_users.new_password is varchar(32).
	$sql = "UPDATE pm_users SET new_password = '". secure_sql($new_pass_hash) ."', activation_key = '". secure_sql($activation_key) ."' WHERE email= '". secure_sql($email_address) ."'";
...
function get_user_data($user_id = false, $username = false)
{
	...
	$user_data['videos_count'] = count_entries('pm_videos', 'submitted', secure_sql($user_data['username'])  ."' AND video_type = ". IS_VIDEO ." AND added <= '". $time_now_minute ); // @todo replace `submitted` with `submitted_user_id`
...
function pm_password_hash($password = '', $algo = PASSWORD_DEFAULT, $options = array())
{
	return password_hash($password, $algo, $options);
}

function pm_password_verify($password = '', $hash = '')
{
