...
//SidenavFit
function resizeMainSidebar() {  

	var sidebar_main_h = $('.nav-sidebar-main').height();
	var sidebar_content_h = $('.sidebar-content').height();
	var sidebar_footer_h = $('.nav-sidebar-footer').height();

	//console.log(sidebar_content_h-sidebar_footer_h);

	if( (sidebar_content_h-sidebar_footer_h) < 670 || sidebar_main_h > 800) { // Previously 670
		$('.nav.nav-sidebar-main').addClass('nav-v-scroll'); 
	}
	else {
		$('.nav.nav-sidebar-main').removeClass('nav-v-scroll');  
	}
}

$(document).ready(function() {
	resizeMainSidebar();
	$(window).resize(resizeMainSidebar);
	$('.nav-item-submenu').click(function() { resizeMainSidebar(); });
});
...
$(document).ajaxError(function(event, jqxhr, settings, thrownError) {
	if (thrownError != "" && thrownError != "abort" && thrownError != "canceled" && jqxhr.status != 0) { 
		$.notify({message: "Action could not be completed.<br />Please reload the page and try again.<br /><br /><code>AJAX error: " + jqxhr.status + " "+ jqxhr.responseText +"<br />" + thrownError +"</code>"}, {type: 'warning'});
	}
});
...
$('.pm-show-loader').click(function() { 
	$('#loading-large').show().find('.loading-msg').replaceWith('Loading...');
	$(".content").css({"opacity" : "0.5"});

	setTimeout(function () {
		$('#loading-large').fadeOut(300);
...
$(document).ready(function() {
	if ( $("#stack-controls").length ) {
		$('#stack-controls').scrollToFixed({
			bottom: 0,
			limit: $('#stack-controls').offset().top,
			postFixed: function() { $(this).removeClass('sticky-preFixed').addClass('sticky-postFixed'); },
			preFixed: function() { $(this).removeClass('sticky-postFixed').addClass('sticky-preFixed'); }
		});
	};
});
<?php endif; ?>
...
<?php if($load_ibutton == 1): ?>
	...
	$(document).ready(function() {
		...
		$("#checkall").click(function () {
			...
			if($('.on_off :checkbox').prop("checked")) {
...
<?php endif; ?>
	...
	$(document).ready(function () {
		 $("input[id^='featured'][type=checkbox]").change(function () { $('#value-featured').text('updated').addClass('bg-success text-light'); });
		 $("input[id^='visibility'][type=radio]").change(function () { $('#value-visibility').text('updated').addClass('bg-success text-light'); });
		 $("input[id^='restricted'][type=radio]").change(function () { $('#value-register').text('updated').addClass('bg-success text-light'); });
		 $("input[class^='pubDate']").change(function () { $('#value-publish').text('updated').addClass('bg-success text-light'); });
		 $("select[class^='pubDate']").change(function () { $('#value-publish').text('updated').addClass('bg-success text-light'); });
		 $("input[id^='site_views_input']").change(function () { $('#value-views').text('updated').addClass('bg-success text-light'); });
		 $("input[id^='submitted']").change(function () { $('#value-submitted').text('updated').addClass('bg-success text-light'); });
		 $("input[id^='allow_comments']").change(function () { $('#value-comments').text('updated').addClass('bg-success text-light'); });
		 $("input[id^='yt_length']").change(function () { $('#value-yt_length').text('updated').addClass('bg-success text-light'); });
		 $("input[id^='show_in_menu'][type=radio]").change(function () { $('#value-showinmenu').text('updated').addClass('bg-success text-light'); });
		 $("input[id^='allow_embedding']").change(function () { $('#value-embedding').text('updated').addClass('bg-success text-light'); });
		 $("input[name^='channel_verified']").change(function () { $('#value-channel-verified').text('updated').addClass('bg-success text-light'); });
		 $("input[name^='channel_featured']").change(function () { $('#value-channel-featured').text('updated').addClass('bg-success text-light'); });
		 $("input[name^='channel_slug']").change(function () { $('#value-channel-permalink').text('updated').addClass('bg-success text-light'); });

		 $("input[name^='seasons_input']").change(function () { $('#value-seasons').text('updated').addClass('bg-success text-light'); });
		 $("input[name^='episodes_input']").change(function () { $('#value-episodes').text('updated').addClass('bg-success text-light'); });
		 $("input[name^='release_year_input']").change(function () { $('#value-release_year').text('updated').addClass('bg-success text-light'); });
...
*/
	...
	$('.search-query').focusin(function() {
		$(".content").css({"opacity" : "0.5"});
		$(this).closest('.input-group-search').addClass("input-group-search-active");
	});
	$('.search-query').focusout(function() {
		$(".content").css({"opacity" : "1.0"});
		$(this).closest('.input-group-search').removeClass("input-group-search-active");
	});

	$('#submitFind').click(function() {
		$('#loading-large').show().find('.loading-msg').replaceWith('Searching');
		$(".pm-tables").css({"opacity" : "0.5"});
		$("i").removeClass("mi-search").addClass("icon-spinner2 spinner");
...
<?php if($load_chzn_drop == 1): ?>
	$(document).ready(function() {
		...
		$(".chzn-select").chosen({width: "100%"}).change(function () { 
			$('li.search-choice').find('span').each(function(){
...
*/
	...
	*/
	...
	$('#show-help-assist').click(function() { $('#help-assist').slideToggle(0); $('#show-help-assist').toggleClass('text-muted'); return false; });
	$('#show-help-link-assist').click(function() { $('#help-assist').slideToggle('fast'); $('#show-help-link-assist').toggleClass('text-muted'); return false; });	
...
<?php endif; ?>
	...
	function validateFormOnSubmit(theForm, say_reason) {
		...
		$("input, textarea").each(function () {
			if ($(this).hasClass('form-required')) { // #must field
		...
		var chosen_selectors = [
			'main_select_category',
			'main_select_genres',
			'main_select_episode_series'
		];
		var chosen_selectors_msg = [
			'Please select at least one category',
			'Please select at least one genre',
			'Please select a series for this episode'
		];
		
		for (i = 0; i < chosen_selectors.length; i++)
		{
			if (document.querySelector('#'+ chosen_selectors[i]) !== null) {
				if ($('#'+ chosen_selectors[i]).val().length == 0) {
					$.notify({message: chosen_selectors_msg[i]}, {type: 'warning'}); 
					$('.chosen-choices').addClass('form-required border-danger');
					$('.chosen-single').addClass('form-required border-danger');
					counter++;
				} else {
					$('.chosen-container').removeClass('chosen-with-drop chosen-active');
					$('.chosen-choices').removeClass('form-required border-danger');
					$('.chosen-single').removeClass('form-required border-danger');
				}
			}
		}

		if (counter > 0) {
			$.notify({message: say_reason}, {type: 'warning'});
			return false;
		}
		return true;
	}
	...
	// 	if ($('#main_select_category').val().length == 0) {
	// 		// if (counter == 0) {
	// 			$.notify({message: 'Please select at least one category'}, {type: 'warning'}); 
	// 			$('.chosen-choices').addClass('form-required border-danger');
	// 			//$('.chosen-container').addClass('chosen-with-drop chosen-active');
	// 			//$('.chosen-drop').show();
	// 			counter++;
	// 			return false;
	// 		// } else {
 ...
 .click(function(event) {
  ...
  $('button[name="add_category_submit_btn"]').click(function(event){
	...
	if (category_name === "" || category_name == $('input[name="add_category_name"]').attr('placeholder')) {
		...
		if (current_page == "edit-article.php") {
			var ajax_do = "add-article-category"; 
			var parent_select_name = 'categories[]';
		} else if (current_page == "edit-series.php") {
			var ajax_do = "add-series-genre"; 
			var parent_select_name = 'genres[]';
		} else {
  ...
  $('.category_mark_featured').click(function(event){
	...
	$.ajax({
		...
		data: {
			...
			"type": $(this).attr('data-category-type')
		},
		type: 'POST',
		dataType: 'json',
		beforeSend: function() {
  ...
  $('#delete-category-image').click(function(event){
	...
	$.ajax({
		...
		$('#category-image-container').html('<div id="show-cat-cover"><img src="img/no-category-thumbnail.png" class="img-responsive"></div>');
...
$(function () {
	...
	.bind('fileuploaddone', function (e, data) { 
		...
		$.each(data.files, function (index, file) {
			...
			if (server_response.success == true) {
				...
				window.location.href = phpmelody.admin_url + '/add-video.php?step=2&filename='+ server_response.filename;
			}
		});
	})
	...
	$('#upload-video-addvideo-btn').fileupload(
		$.extend({}, pm_fileupload_single_options, {
			dropZone: $('#upload-video-modal-dropzone'),
			acceptFileTypes: pm_media_file_types
		})
	...
	.bind('fileuploadadd', function (e, data) {
		...
		pm_fileupload_add(e, data);
		data.formData = $('#upload-video-addvideo-page').serializeArray();
	...
	.bind('fileuploaddone', function (e, data) {
		$.each(data.files, function (index, file) {
			...
			if (server_response.success == true) {
				if ($("#wysiwygtextarea-WYSIWYG, #textarea-WYSIWYG_ifr").length > 0) {
					$("#wysiwygtextarea-WYSIWYG, #textarea-WYSIWYG_ifr").contents().find("body").append(server_response.html);
				} else if ($("#textarea-WYSIWYG, #textarea-WYSIWYG_ifr").length > 0) {
					var textarea = $("#textarea-WYSIWYG, #textarea-WYSIWYG_ifr").val();
					$("#textarea-WYSIWYG, #textarea-WYSIWYG_ifr").val(textarea + server_response.html);
				}
			}
		});
	...
	.bind('fileuploadadd', function (e, data) {
		...
		if (language == '') {
			$.notify({message: 'Choose a LANGUAGE before uploading the file.'}, {type: 'warning'});
	...
	/*
	 * Series poster uploader
	 */ 
	<?php if ($showm == 'mod_series'): ?>
	$('#upload-series-image-btn').fileupload(
		$.extend({}, pm_fileupload_single_options, {
			dropZone: $('#series-image-dropzone')
		})
	)
	.bind('fileuploadadd', function (e, data) {
		$('input[name="upload-type"]').val('series-image');
		$('input[name="do"]').val('upload-image');
	...
	.bind('fileuploaddone', function (e, data) { 
		$.each(data.files, function (index, file) {
			...
			if (server_response.success == true) {
				$('#series-image-container').html(server_response.html);
			}
		});
	...
	<?php if ($showm == 'mod_article'): ?>

	/*
	 * Article thumbnail uploader
	 */
	$('#upload-article-thumb-btn').fileupload(
		$.extend({}, pm_fileupload_single_options, {
			dropZone: $('#article-thumb-dropzone')
		})
	)
	.bind('fileuploadadd', function (e, data) {
		
		$('input[name="upload-type"]').val('article-thumb');
		$('input[name="do"]').val('upload-image');
	...
	.bind('fileuploaddone', function (e, data) { 
		$.each(data.files, function (index, file) {
			...
			if (server_response.success == true) {
				$('#article-thumb-container').html(server_response.html);
			}
		});
...
<?php if($load_tinymce == 1 && _SEOMOD): ?>
	...
	if ( $(".permalink-input").length ) {

		var inputPermalink = $('.permalink-input').val().length;
		if(inputPermalink > 0) {
			$('input[class="permalink-input"]').attr('size', inputPermalink).css('width', inputPermalink * 7 +'px');
		}
...
$(document).bind('keydown', 'alt+l', function() {
	window.location = 'theme-settings.php';
	return false;
});
...
$(document).bind('keydown', 'shift+a', function() {
	$(".pm-tables").each(function(){
		if ( $('input:checkbox').prop('checked', true)) { // changed from attr('checked')
	...
	$("#import-opt-content").each(function(){
		...
		if ( $('input[name^="video_ids"]:checkbox').prop('checked', true)) { // changed from attr('checked')
		...
		$('.on_off :checkbox').iButton("repaint");
		if($('.on_off :checkbox').attr("checked")) {
			$('.video-stack').addClass("stack-selected");
		}else {
			$('.video-stack').removeClass("stack-selected");
		}
			...
			$.ajax({
				...
				success: function(data){
					if (data['type'] == 'success') {
						$('#update-response-'+ meta_id).html(data['html']).show().delay(3000).fadeOut("slow");
					} else { 
...
$(document).ready(function(){
	$(".gautocomplete").googleSuggest({service: "youtube"}).keyup(function(e) { 
		if (e.which === 13) $('#search-videos-btn').click();   // enter 
		e.preventDefault();
		e.stopPropagation();
	});
});
...
<?php if($showm == '8') : // Settings page ?>
	...
	$(document).ready(function() {
		...
		$('#player_select').on('change', function(){
			var playerval = $(this).val(); 
			$(".hide-player-opt").hide();
			$("#show_"+playerval).show().addClass('animated flash');
		});
	});
...
<script type="text/javascript">
$(document).ready(function() {
	$('.input-select-series').change(function(){

		if ($(this).prop("checked")) {
			$(this).closest('.card-series').addClass("series-selected");
		}
		else {
			$(this).closest('.card-series').removeClass("series-selected");
		}
	});
});
</script>
...
<?php include('modals/modal-update-database.php');?>
<?php endif; ?>