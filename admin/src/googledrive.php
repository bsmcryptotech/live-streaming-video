...
namespace phpmelody\sources\src_googledrive;
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		...
		if ( ! $title)
		{
			if (strpos($video_data[$i], 'og:title'))
			{
				...
				{
					$remove_extensions = array('.flv', '.mp4', '.mov', '.wmv', '.divx', '.avi', '.mkv', '.asf', '.wma', '.mp3', '.m4v', '.m4a', '.3gp', '.3g2');
					$info['video_title'] = str_replace($remove_extensions, '', $matches[1]);
					$title = true;
				}
			}
		}
		...
		if ( ! $id)
		{
			if (preg_match('/\/d\/[A-Z-a-z0-9._-]*/', $video_data[$i], $matches) != 0)
			{
		...
		if ( ! $thumb)
		{
			...
			{
				if (preg_match('/og:image" content="(.*?)"/', $video_data[$i], $matches) != 0)
				{
		...
		if ( ! $direct)
		{
			$info['direct'] = 'https://drive.google.com/uc?export=download&id='. $info['yt_id'];
			$direct = true;
		}
		...
		if ( ! $duration)
		{
			...
			if (strpos($video_data[$i], 'length_seconds'))
			{
				if (preg_match('/length_seconds", "(.*?)"/', $video_data[$i], $matches) != 0)
				{
		...
		if ($title && $id && $thumb && $direct && $duration)
		{
			break;
		}
