...
namespace phpmelody\sources\src_imgur;

function get_info($url)
{
	$video_data = array();
	
	$strip_url = array("i.", "/r", "/a", "/gallery", ".gif", ".gifv", ".mp4", ".webm");
	$target_url  = str_replace($strip_url, '', $url);
	...
	for ($i = 0; $i < $arr_length; $i++)
	{
		...
		if ( ! $id)
		{
			...
			if (strpos($video_data[$i], '"og:image"'))
			{
				...
				{
					$pieces = explode('/', $matches[0]); // Deconstruct URL
					$uniq_id = explode('.', $pieces[3]); // Image File Name

					$info['yt_id'] = trim($uniq_id[0]);
					$yt_id = true;
				}
			}
		}
