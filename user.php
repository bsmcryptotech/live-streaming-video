...
$query_total = @mysql_query("SELECT COUNT(*) as total FROM pm_videos WHERE submitted = '". secure_sql($profile_data['username']) ."' AND added <= '". time() ."' AND video_type = ". IS_VIDEO);
...
if ($userdata['id'] == $profile_data['id'])
{
	...
	if ($result)
	{
		...
		if (pm_count($pending_video_list) > 0)
		{
...
if ($userdata['id'] == $profile_data['id'])
{
	$total_playlists = pm_count($userdata['playlists']);
}
else
{
